...
const char* PushBulletAPIKEY = "o.oLXohEmk6dJ10roVDOCMAXJwjhycxhzi"; //chave do pushbullet
...
Thread threadDeNotificaoTempUmi;
void chama_notificacoes() {
  if(t >= 55){
    notificar_temp_elevadao();
  }
  if(h <= 30){
...
void func_RFID() {
  ...
  if ( mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    content.toUpperCase();
    Serial.println();
    if (content.substring(1) == "D5 3C B1 79") { //change UID of the card that you want to give access
      Serial.println(" Acesso Autorizado ");
      Serial.println(" Bem-vindo Chaveiro ");
    }
    else if (content.substring(1) == "34 37 38 05") { //change UID of the card that you want to give access
      Serial.println(" Acesso Autorizado ");
      Serial.println(" Bem-vindo Hugo ");
      notificar_hugo_rfid(); //chama notificação
    }
    else {
...
void movimento() {
  ...
  if (val_sensor_mov == 1) {
    notificar_sensor_movimento(); //chama uma notificacao se tiveer movimento
  }
}
...
void notificar_hugo_rfid() {
  ...
  String url = "/v2/pushes";
  String messagebody = "{\"type\": \"note\", \"title\": \"Projeto Final IoT\", \"body\": \"Hugo Identificado!\"}\r\n";
  ...
  client.print(messagebody);
  Serial.println();
  Serial.println("Notificacao de Hugo enviada");
}
...
void notificar_umidade_baixa() {
  ...
  String url = "/v2/pushes";
  String messagebody = "{\"type\": \"note\", \"title\": \"Projeto Final IoT\", \"body\": \"Umidade muito baixa, se hidrate!!\"}\r\n";
...
void setup() {
  ...
  threadDeNotificaoTempUmi.setInterval(5000);
  threadDeNotificaoTempUmi.onRun(chama_notificacoes);
  ...
  Todas_as_Threads.add(&threadDeNotificaoTempUmi);
